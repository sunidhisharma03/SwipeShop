...
unmapped_types = interactions[interactions['interaction_score'].isnull(
)]['interaction_type'].unique()
if len(unmapped_types) > 0:
    raise ValueError(
        f"Some interaction types were not mapped correctly to scores: {unmapped_types}")
...
def preprocess_products(products):
    ...
    # Drop these columns if not used in features
    products = products.drop(columns=['category', 'video_id'])
...
aggregated_interactions = interactions.groupby(['user_id', 'product_id'])[
    'interaction_score'].mean().reset_index()

# Create user-item interaction matrix
user_item_matrix = aggregated_interactions.pivot(
    index='user_id', columns='product_id', values='interaction_score').fillna(0)
